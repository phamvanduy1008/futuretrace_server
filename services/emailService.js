...
console.log('✉️ [Brevo Init] Khởi tạo dịch vụ email qua Brevo HTTP API.');
...
const sendOtpEmail = async (email, otp) => {
  ...
  console.log(`✉️ [Brevo API Send] Đang gửi OTP tới ${email}`);
  try {
    const apiKey = process.env.BREVO_API_KEY;
    if (!apiKey) {
      throw new Error('Thiếu BREVO_API_KEY trong cấu hình môi trường.');
    }

    const fromEmail = process.env.BREVO_FROM_EMAIL;
    if (!fromEmail) {
      throw new Error('Thiếu BREVO_FROM_EMAIL trong cấu hình môi trường.');
    }
    ...
    console.error(`❌ [Brevo API Send] Gửi email thất bại tới ${email}:`, 
